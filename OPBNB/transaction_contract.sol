...
contract Nebula3Gamefi is ERC20, Ownable {
    ...
    function mintWithMemo(address to, uint256 amount, string memory memo) public onlyAuthorized {
        require(bytes(memo).length <= 64, "Memo too long");
        _mint(to, amount);
        emit MintWithMemo(to, amount, memo);
    }

    function burnWithMemo(uint256 amount, string memory memo) public onlyAuthorized {
        require(bytes(memo).length <= 64, "Memo too long");
